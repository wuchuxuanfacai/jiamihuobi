...
7. 使用 target-position 目标仓位追踪，避免重复叠仓。
...
14. 不要生成、引用或修复任何名为 `positions` 的裸变量。这个策略不需要读取平台持仓列表；它用 `_target_position_qty` 追踪目标仓位。

如果你看到 `name 'positions' is not defined`，不要继续在错误代码上打补丁；请回到仓库原始目录结构，完整读取 `getagent_playbook/`，不要让系统把源码改写成单文件策略。
