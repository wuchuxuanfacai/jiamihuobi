...
This Playbook trades BTCUSDT perpetual futures with a selective target-size
...
present. Trade size is scaled down when realized volatility is high, so the
strategy does not simply use maximum exposure every time a bearish condition
appears.

The long model can open only a small recovery exposure when upside alignment is
...
weakens or volatility-adjusted sizing no longer supports the exposure. Range
exposure closes when price leaves the channel extreme or a clearer trend takes
over.

Orders in the replay are target-size adjustments, not independent signal
...
larger exposure in confirmed regimes, while lower values make the strategy
...
trade, not the whole strategy equity curve. The package declares maker and
